...
writerObj1 = VideoWriter('perm1e-17_diff_CNM');
...
P.phi_0 = interp1(T_interp,phi_interp,min(1,max(0,(P.T_0-273)/175)));
...
for k=1:Nt
    ...
    RHS = ((P.omega-1)*ones(size(S.phiold))+(P.omega*(P.omega-1)*ones(size(S.phiold))./(P.omega*(ones(size(S.phiold))-S.phiold)+S.phiold).^2))/2.*phi_cdx.*T_cdx;
    RHS = RHS+((ones(size(S.phiold))-(1-P.omega)*S.phiold)+(P.omega*ones(size(S.phiold))./(P.omega*(ones(size(S.phiold))-S.phiold)+S.phiold)))/2.*T_xx;
    
    RHS = RHS+ ((P.omega-1)*ones(size(S.phiold))+(P.omega*(P.omega-1)*ones(size(S.phiold))./(P.omega*(ones(size(S.phiold))-S.phiold)+S.phiold).^2))/2.*phi_cdy.*T_cdy;
    RHS = RHS+((ones(size(S.phiold))-(1-P.omega)*S.phiold)+(P.omega*ones(size(S.phiold))./(P.omega*(ones(size(S.phiold))-S.phiold)+S.phiold)))/2.*T_yy;
     
    RHS = RHS + (S.phiold-ones(size(S.phiold))).*(S.w(:,:,1).*T_cdx+S.w(:,:,2).*T_cdy);
    
    RHS = RHS -(P.alpha*ones(size(S.T))+P.nu*S.T).*(S.phi-S.phiold)/dt;
    RHS = RHS + P.Diff/P.D*(Dxx(S.phiold,h)+Dyy(S.phiold,h)).*(S.T+P.alpha/P.nu*ones(size(S.T)));
   ...
   % Momentum Balance (7.5.2)
    S.w(:,:,1)=-P.kappa_perp_hat*kappa_scale(P,S.T).*pi_cdx./mu_hat(P,S.T)./(ones(size(S.phi))-S.phi);
    S.w(:,:,2)=-P.kappa_par_hat*kappa_scale(P,S.T).*pi_cdy./mu_hat(P,S.T)./(ones(size(S.phi))-S.phi);
    ...
    w_1_right = S.w(:,end,1);
    w_2_top = S.w(1,:,2);
    w_2_bottom = S.w(end,:,2);
        % No inward Flux Condition
    w_1_left(w_1_left>0) = 0;
    w_1_right(w_1_right<0) = 0;
    w_2_top(w_2_top>0) = 0;
    w_2_bottom(w_2_bottom<0) = 0;
    
    S.w(:,1,1) = w_1_left;
    S.w(:,end,1) = w_1_right;
    S.w(1,:,2) = w_2_top;
    S.w(end,:,2) = w_2_bottom;
    ...
    %Left/Right T & Phi
    for j=2:P.Ny-1
        S.phi(j,1)= interp1(T_interp,phi_interp,min(1,max(0,(S.Told(j,1)*(P.T_D-P.T_0)+P.T_0-273)/175))); % Use previous T on boundary to interpolate preprocessed root data
        S.phi(j,P.Nx)= interp1(T_interp,phi_interp,min(1,max(0,(S.Told(j,P.Nx)*(P.T_D-P.T_0)+P.T_0-273)/175))); % Use previous T on boundary to interpolate preprocessed root data

            % set T on boundary (using old T in j_evap)
            %temp = (P.T_0-293)/(P.T_D-P.T_0); % Room temperature
             temp = (P.T_0-100-273)/(P.T_D-P.T_0); % hot surface B.L. temperature
            a=2/(h(j,1,1)+h(j,2,1))/(P.omega*(1-S.phi(j,1))+S.phi(j,1)); %helpful coefficient
            a=a/2+2/(h(j,1,1)+h(j,2,1))*((1-S.phi(j,1))/P.omega+S.phi(j,1));
            S.T(j,1)=max(S.Told(j,1),(S.T(j,2)*a-temp-j_evap(P,S.Told(j,1),S.phi(j,1),t)) / (1+a));     
            a=2/(h(j,P.Nx,1)+h(j,P.Nx-1,1))*((1-S.phi(j,P.Nx))/P.omega+S.phi(j,P.Nx));
            a=a/2+2/(h(j,P.Nx,1)+h(j,P.Nx-1,1))/(P.omega*(1-S.phi(j,P.Nx))+S.phi(j,P.Nx));

            S.T(j,P.Nx)=max(S.Told(j,P.Nx),(S.T(j,P.Nx-1)*a-temp-j_evap(P,S.Told(j,P.Nx),S.phi(j,P.Nx),t)) / (1+a));
...
% take corners to be average of adjacent points
    ...
    h(P.Ny,:,1) = h_0*(P.phi_0*(min(S.phi(P.Ny,:),0.4347*ones(size(S.phi(P.Ny,:))))).^(-1).*xi(P,S.T(P.Ny,:))).^(1/2);
    h(P.Ny,:,2) = h_0*(P.phi_0*(min(S.phi(P.Ny,:),0.4347*ones(size(S.phi(P.Ny,:))))).^(-1)./xi(P,S.T(P.Ny,:))).^(1/2);
    
    %Top
    h(1,:,1) = h_0*(P.phi_0*(min(S.phi(1,:),0.4347*ones(size(S.phi(1,:))))).^(-1).*xi(P,S.T(1,:))).^(1/2);
    h(1,:,2) = h_0*(P.phi_0*(min(S.phi(1,:),0.4347*ones(size(S.phi(1,:))))).^(-1)./xi(P,S.T(1,:))).^(1/2);
    
    %Left
    h(:,1,1) = h_0*(P.phi_0*(min(S.phi(:,1),0.4347*ones(size(S.phi(:,1))))).^(-1).*xi(P,S.T(:,1))).^(1/2);
    h(:,1,2) = h_0*(P.phi_0*(min(S.phi(:,1),0.4347*ones(size(S.phi(:,1))))).^(-1)./xi(P,S.T(:,1))).^(1/2);
    %Right
    h(:,P.Nx,1) = h_0*(P.phi_0*(min(S.phi(:,P.Nx),0.4347*ones(size(S.phi(:,P.Nx))))).^(-1).*xi(P,S.T(:,P.Nx))).^(1/2);
    h(:,P.Nx,2) = h_0*(P.phi_0*(min(S.phi(:,P.Nx),0.4347*ones(size(S.phi(:,P.Nx))))).^(-1)./xi(P,S.T(:,P.Nx))).^(1/2);
