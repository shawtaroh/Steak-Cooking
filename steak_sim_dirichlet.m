...
%
%
function steak_sim_dirichlet( P )
addpath('./steaksrc/');
...
writerObj1 = VideoWriter('steak_movie_dirichlet');
...
P.phi_0 = interp1(T_interp,phi_interp,min(1,(P.T_0-273)/175));
...
for k=1:Nt
    ...
    RHS = P.omega*(P.omega-1)*ones(size(S.phiold))./(P.omega*(ones(size(S.phiold))-S.phiold)+S.phiold).^2;
    RHS = RHS.*phi_cdx.*T_cdx;
    RHS = RHS+P.omega*ones(size(S.phiold))./(P.omega*(ones(size(S.phiold))-S.phiold)+S.phiold).*T_xx;
   ...
   % Momentum Balance (7.5.2)
    ...
    %Nx = P.Nx;
    %Ny = P.Ny;
    %    S.w(:,1,1) = S.w(:,1,1) - j_evap_momentum(P,S.T(:,1),S.phi(:,1),t)./(ones(size(S.phi(:,1)))-S.phi(:,1));
    %    S.w(:,Nx,1) = S.w(:,Nx,1) +
    %    j_evap_momentum(P,S.T(:,Nx),S.phi(:,Nx),t)./(ones(size(S.phi(:,1)))-S.phi(:,Nx));

    %    S.w(1,:,2) = S.w(1,:,2) - j_evap_momentum(P,S.T(1,:),S.phi(1,:),t)./(ones(size(S.phi(1,:)))-S.phi(1,:));
    %    S.w(Ny,:,2) = S.w(Ny,:,2) + j_evap_momentum(P,S.T(Ny,:),S.phi(Ny,:),t)./(ones(size(S.phi(1,:)))-S.phi(Ny,:));
    ...
    %Top/Bottom T & Phi
    for i=2:P.Nx-1
            S.phi(1,i)= interp1(T_interp,phi_interp,min(1,(S.Told(1,i)*(P.T_D-P.T_0)+P.T_0-273)/175)); % Use previous T on boundary to interpolate preprocessed root data
            S.phi(P.Ny,i)= interp1(T_interp,phi_interp,min(1,(S.Told(P.Ny,i)*(P.T_D-P.T_0)+P.T_0-273)/175)); % Use previous T on boundary to interpolate preprocessed root data
            %S.phi(1,i) = S.phi(2,i);
    ...
    %Left/Right T & Phi
    for j=2:P.Ny-1
        S.phi(j,1)= interp1(T_interp,phi_interp,min(1,(S.Told(j,1)*(P.T_D-P.T_0)+P.T_0-273)/175)); % Use previous T on boundary to interpolate preprocessed root data
        S.phi(j,P.Nx)= interp1(T_interp,phi_interp,min(1,(S.Told(j,P.Nx)*(P.T_D-P.T_0)+P.T_0-273)/175)); % Use previous T on boundary to interpolate preprocessed root data
        %    S.phi(j,1) = S.phi(j,2);
