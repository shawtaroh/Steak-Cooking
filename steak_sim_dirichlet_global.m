...
%
%
function steak_sim_fastest_global( P )
addpath('./steaksrc/');
...
writerObj1 = VideoWriter('steak_movie_global');
...
for k=1:Nt
    ...
    S.phi = S.phiold+dt*div_phi_w_cd;
    ...
    %Top/Bottom T & Phi
    for i=2:P.Nx-1
            %S.phi(1,i)= interp1(T_interp,phi_interp,min(S.Told(1,i)/175*P.T_D,1)); % Use previous T on boundary to interpolate preprocessed root data
            %S.phi(P.Ny,i)= interp1(T_interp,phi_interp,min(S.Told(P.Ny,i)/175*P.T_D,1)); % Use previous T on boundary to interpolate preprocessed root data
            S.phi(1,i) = S.phi(2,i);
    ...
    %Left/Right T & Phi
    for j=2:P.Ny-1
        %S.phi(j,1)= interp1(T_interp,phi_interp,min(S.Told(j,1)/175*P.T_D,1)); % Use previous T on boundary to interpolate preprocessed root data
        %S.phi(j,P.Nx)= interp1(T_interp,phi_interp,min(S.Told(j,P.Nx)/175*P.T_D,1)); % Use previous T on boundary to interpolate preprocessed root data
        S.phi(j,1) = S.phi(j,2);
...
% take corners to be average of adjacent points
    ...
    h(:,:,2) = h_0*(P.phi_0/phi_avg/xi_avg)^(1/3)*ones(P.Ny,P.Nx);
    h(:,:,1) = h_0*(P.phi_0/phi_avg*xi_avg)^(1/3)*ones(P.Ny,P.Nx);
